...
DATA = [
    {
        "pose_path": os.path.join(EXAMPLE_DATA_DIR_LOCAL, f"poses.npy"),
        "intrinsic_path": os.path.join(EXAMPLE_DATA_DIR_LOCAL, f"intrinsics.npy"),
        "first_frame_path": os.path.join(EXAMPLE_DATA_DIR_LOCAL, f"image.jpg"),
        "text_prompt_path": os.path.join(EXAMPLE_DATA_DIR_LOCAL, f"prompt.txt"),
    }
...
print(
    f"Running Lingbot World inference with config: {CONFIG_NAME}"
)
...
for data in DATA:
    ...
    plucker_video = get_plucker_embeddings(c2ws, Ks, args.video_height, args.video_width)
...
# initialize pipeline
pipeline = LINGBOT_WORLD_CONFIGS[CONFIG_NAME].setup(device=device)
