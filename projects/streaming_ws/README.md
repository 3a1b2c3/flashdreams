...
```bash
python -m projects.streaming_ws server
```

Terminal B — connect and stream (defaults to `ws://127.0.0.1:8765`):

```bash
python -m projects.streaming_ws client
```

Useful flags (see full lists with `--help`):

```bash
python -m projects.streaming_ws server --help
python -m projects.streaming_ws client --help
python -m projects.streaming_ws client --uri ws://<host>:8765 --show-window
```
...
```bash
python -m projects.streaming_ws viser
```
...
```bash
python -m projects.streaming_ws viser --flat-background-depth-m 10.0
```

```bash
python -m projects.streaming_ws viser --help
```
