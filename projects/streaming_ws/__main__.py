"""CLI: ``python -m projects.streaming_ws {server|client|viser}``.
...
def main() -> None:
    if len(sys.argv) < 2 or sys.argv[1] not in ("server", "client", "viser"):
        print(
            "usage: python -m projects.streaming_ws {server|client|viser} [options...]",
            file=sys.stderr,
        )
        sys.exit(2)
    cmd = sys.argv[1]
    ...
    if cmd == "server":
        ...
        from projects.streaming_ws.viser_app import ViserOnlyConfig, main_viser
